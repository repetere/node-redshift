...
ORM.prototype.update = function (whereClause, data, callback) {
  ...
  const query = generateQuery.makeUpdate(this.tableName, whereClause, data);

  this.redshiftClient.parameterizedQuery(query.toString(), query.values, {}, callback);
};
...
ORM.prototype.query = function(options, callback) {
  const query = generateQuery.makeQuery(this.tableName, options)
  this.redshiftClient.parameterizedQuery(query.toString(), query.values, {}, callback);
}
